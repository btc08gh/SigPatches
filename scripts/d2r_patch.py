...
with open('main', 'rb') as fi:
    read_data = fi.read()
    result = re.search(
        b'\xd2...\xf2...\xf2...\xcb...\xeb.......\x14', read_data)
    patch = "%06X%s%s" % (result.end(), "0004", "1F2003D5")
    text_file = open(
        "SigPatches/atmosphere/exefs_patches/d2r_offline_fix/" + get_build_id() + ".ips", "wb")
    ...
    text_file = open("cheats/atmosphere/contents/0100726014352000/cheats/" +
                     get_build_id()[:16] + ".txt", "w")
